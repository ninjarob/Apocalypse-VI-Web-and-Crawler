...
function seedData() {
  ...
  // Seed sample rooms and exits (current 3 rooms from database)
  const sampleRooms = [
    {
      id: 1,
      zone_id: 2,
  ...
  // Seed room exits for the sample rooms
  const sampleRoomExits = [
    // Market Square (room 1) exits
    {
      from_room_id: 1,
      to_room_id: 2,
      direction: 'north',
      description: 'South Temple Street',
      look_description: 'You see the southern temple square.',
      is_zone_exit: 0
    },
    {
      from_room_id: 1,
      to_room_id: null,
      direction: 'east',
      description: 'Main Street East',
      look_description: 'You see the main street.',
      is_zone_exit: 0
    },
    {
      from_room_id: 1,
      to_room_id: null,
      direction: 'south',
      description: 'Northern Bazaar',
      look_description: 'You see the bazaar.',
      is_zone_exit: 0
    },
    {
      from_room_id: 1,
      to_room_id: null,
      direction: 'west',
      description: 'Main Street West',
      look_description: 'You see the main street.',
      is_zone_exit: 0
    },
    // South Temple Street (room 2) exits
    {
      from_room_id: 2,
      to_room_id: 1,
      direction: 'south',
      description: 'Market Square',
      look_description: 'You see the Market Square.',
      is_zone_exit: 0
    },
    {
      from_room_id: 2,
      to_room_id: null,
      direction: 'north',
      description: 'North Temple Street',
      look_description: 'The temple continues.',
      is_zone_exit: 0
    },
    // Outside the City Walls (room 3) exits
    {
      from_room_id: 3,
      to_room_id: null,
      direction: 'north',
      description: 'Grasslands near the walls of Midgaard',
      look_description: 'Beautiful grasslands lie north.',
      is_zone_exit: 0
    },
    {
      from_room_id: 3,
      to_room_id: null,
      direction: 'south',
      description: 'Rear exit of the Temple',
      look_description: 'The rear exit of the Temple',
      is_zone_exit: 1
    }
  ];

  const insertRoomExit = db.prepare('INSERT INTO room_exits (from_room_id, to_room_id, direction, description, look_description, is_zone_exit) VALUES (?, ?, ?, ?, ?, ?)');

  sampleRoomExits.forEach(exit => {
    insertRoomExit.run(exit.from_room_id, exit.to_room_id, exit.direction, exit.description, exit.look_description, exit.is_zone_exit);
  });

  insertRoomExit.finalize(() => {
    console.log(`  ✓ Seeded ${sampleRoomExits.length} room exits`);
    checkComplete();
  });
